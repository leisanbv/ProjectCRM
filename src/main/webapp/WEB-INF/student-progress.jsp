    ...
    <section class="semestr-note">
        ...
        <div class="choice-semestr">
            ...
            <form method="get" action="/student-progress">
                    ...
                    <c:forEach items="${terms}" var="term">
                        ...
                        <c:choose>
                            <c:when test="${term.id eq selectedTerm.id}">
                                <option selected value="${term.id}" class="choice"> ${term.name_of_term} </option>
                            </c:when>
                            <c:otherwise>
                                <option value="${term.id}" class="choice"> ${term.name_of_term} </option>
                            </c:otherwise>
                        </c:choose>
                ...
                <br>
                <br>
                <span> <h5>Средняя оценка за семестр:  <fmt:formatNumber type="number" value="${average}" maxFractionDigits="2"></fmt:formatNumber></h5>
                </span>
                <br>
